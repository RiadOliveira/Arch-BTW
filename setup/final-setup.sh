...
openssl req -newkey rsa:2048 -nodes -keyout KEK.key -new -x509 -sha256 -days 3650 -subj "/CN=KEK Key/" -out KEK.crt
openssl req -newkey rsa:2048 -nodes -keyout db.key  -new -x509 -sha256 -days 3650 -subj "/CN=db Key/"  -out db.crt
...
cp /usr/share/efitools/efi/KeyTool.efi /boot/EFI/
mkdir -p /boot/EFI/keys
cp /etc/secureboot/keys/*.auth /boot/EFI/keys/
cp /etc/secureboot/keys/*.esl /boot/EFI/keys/
...
chmod +x /usr/local/bin/set-power-profile.sh

if [[ -v HDPARM_DEVICE ]]; then
  hdparm -B 255 "/dev/$HDPARM_DEVICE"
fi
